...
export function GoogleSignIn({ buttonProps, buttonViewProps }: { buttonProps?: PressableProps; buttonViewProps?: ViewProps }) {
	return (
		<View className={cn('flex-1', buttonViewProps?.className)} {...buttonViewProps}>
			<Pressable
				className={cn(
					'flex-row items-center justify-center gap-2.5 rounded-xl border border-border bg-input py-3.5 active:opacity-75 dark:bg-input/30',
					buttonProps?.className,
				)}
				onPress={async () => {
					await authClient.signIn.social({
						provider: 'google',
						callbackURL: 'filedrive://',
					})
				}}
				{...buttonProps}
			>
				...
				<Text className='font-medium text-foreground text-sm'>Continue with Google</Text>
			</Pressable>
		</View>
	)
}
